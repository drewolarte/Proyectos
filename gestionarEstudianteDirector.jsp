...
<head>
    <title>Gestion Estudiante Director</title>
    <meta charset="utf-8">
...
<body>
    <div class="container">
        ...
        <c:forEach var="row" items="${result.rows}">
            ...
            <c:if test="${row.anteproyecto_estudiante!= null && row.docente_id_docentedirector!=null  && row.aprobacion_director_anteproyecto_estudiante.equals('Aprobado')}">
        ...
        <c:if test="${param.nota!=null}">
            <sql:update var="result1" dataSource="${universidad}">
                update estudiante 
                set aprobacion_director_anteproyecto_estudiante = '${param.nota}' where id_estudiante = ${param.id}
            </sql:update>
            <c:if test="${result1==1}">
                <c:redirect url="gestionarProyectosDirector.jsp"/>
            </c:if>
        </c:if>
