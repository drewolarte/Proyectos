...
<body>
    <div class="container">
        ...
        <h2 class="text-center" id="titulo">LOGIN ESTUDIANTE</h2>
            <form method="POST">
        ...
        <c:if test="${param.modifica != null}">
            ...
            <c:forEach var="row" items="${result.rows}">
                <c:if test="${row.id_estudiante==param.user&&row.contrasena_estudiante==param.pwd}">
                    <c:set value="${row}" var="admin" scope="session"/>
                    <c:redirect url="indexEstudiante.jsp"/>
                </c:if>
            </c:forEach>
            <br>
