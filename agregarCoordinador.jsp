...
<body>
    <div class="container">
        ...
        <br>
        <h1 class="text-center" id="titulo">Agregar Coordinador</h1>
        <br>
        <form method="POST">
                ...
                <select class="form-control" id="rol" name="rol">
                    <c:forEach var="item" items="${rsRol.rows}">
                        <option value="${item.id_roles}" required 
                            <c:if test="${'Coordinador'!=item.descripcion_roles}">
                                disabled
                            </c:if>>
        ...
        <c:if test="${param.id != null}">
            <sql:update var="result" dataSource="${universidad}">
                INSERT INTO
                coordinador
                values ('${param.id}','${param.nombre}','${param.apellido}','${param.contrasena}','${param.rol}')
            </sql:update>
            <c:if test="${result==1}">
