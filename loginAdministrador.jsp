...
<body>
    <div class="container">
        ...
        <h2 class="text-center" id="titulo">LOGIN ADMINISTRADOR</h2>
            <form method="POST">
        ...
        <c:if test="${param.modifica != null}">
            <sql:query var="result" dataSource="${universidad}" sql="SELECT * FROM administrador WHERE id_administrador=? and contrasena_administrador=?">
                <sql:param value="${param.user}"/>
                <sql:param value="${param.pwd}"/>
            </sql:query>
            <c:forEach var="row" items="${result.rows}">
                <c:if test="${row.id_administrador==param.user&&row.contrasena_administrador==param.pwd}">
                    <c:set value="${row}" var="admin" scope="session"/>
                    <c:redirect url="indexAdministrador.jsp"/>
                </c:if>
            </c:forEach>
            <br>
