...
<body>
    <div class="container">
        ...
        <h2 class="text-center" id="titulo">LOGIN COORDINADOR</h2>
            <form method="POST">
        ...
        <c:if test="${param.modifica != null}">
            <sql:query var="result" dataSource="${universidad}" sql="SELECT * FROM coordinador WHERE id_coordinador=? and contrasena_coordinador=?">
                <sql:param value="${param.user}"/>
                <sql:param value="${param.pwd}"/>
            </sql:query>
            <c:forEach var="row" items="${result.rows}">
                <c:if test="${row.id_coordinador==param.user&&row.contrasena_coordinador==param.pwd}">
                    <c:set value="${row}" var="admin" scope="session"/>
                    <c:redirect url="indexCoordinador.jsp"/>
                </c:if>
            </c:forEach>
            <br>
